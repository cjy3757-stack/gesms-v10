/* GESMS V10.7 - unified evangelism workbook + settings module */
(function(){
'use strict';
...
function renderProjects(){
  ...
  el.innerHTML=`<div class="project-month-toolbar"><button class="project-month-btn active" data-month="all">전체 ${projects.length}개</button>${keys.map(k=>`<button class="project-month-btn" data-month="${esc(k)}">${esc(projectMonthLabel(k))} ${grouped[k].length}개</button>`).join('')}</div><div id="projectMonthGroups">${keys.map(k=>`<section class="project-month-section ${k===initial?'':'collapsed'}" data-project-month="${esc(k)}"><div class="project-month-head"><h4>📅 ${esc(projectMonthLabel(k))}</h4><b>${grouped[k].length}개 계획</b></div><div class="project-month-body">${grouped[k].map(projectCard).join('')}</div></section>`).join('')}</div>`;
  el.querySelectorAll('.project-month-head').forEach(h=>h.onclick=()=>h.parentElement.classList.toggle('collapsed'));
  el.querySelectorAll('.project-month-btn').forEach(btn=>btn.onclick=()=>{el.querySelectorAll('.project-month-btn').forEach(x=>x.classList.remove('active'));btn.classList.add('active');const month=btn.dataset.month;el.querySelectorAll('[data-project-month]').forEach(sec=>{sec.style.display=(month==='all'||sec.dataset.projectMonth===month)?'block':'none';if(month!=='all'&&sec.dataset.projectMonth===month)sec.classList.remove('collapsed')})});
...
function apply(data){targets=data.targets||[];projects=data.projects||[];appSettings=data.settings||{};window.PRAYER_TARGETS=targets;window.MISSION_PROJECTS=projects;window.GESMS_SETTINGS=appSettings;renderFilters();renderStats();renderList();renderProjects();const goalSource=Number(appSettings['연간영혼구원목표'])>0?` · 연간목표 ${getGoal()}명(환경설정)`:'';setStatus(`${data.name} · 기도대상 ${targets.length}명 · 프로젝트 ${projects.length}건${goalSource}${data.warning?' · '+data.warning:''}`);}
