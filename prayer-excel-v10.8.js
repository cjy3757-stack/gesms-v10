/* GESMS V10.9.4 Stable - 전도영혼구원 V3.1 단일 원본 연동 */
(function(){
'use strict';
...
async function fetchBuffer(name){
  ...
  let last;
  for(let i=0;i<2;i++){
    try{const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error(`${name}: HTTP ${r.status}`);const b=await r.arrayBuffer();if(b.byteLength<100)throw new Error(`${name}: 파일이 비어 있습니다.`);return b}catch(e){last=e;if(i===0)await new Promise(res=>setTimeout(res,500));}
  }
  throw last;
}
...
async function load(){const embedded=window.GESMS_STATIC_PRAYER_DATA;if(embedded&&Array.isArray(embedded.targets)){apply(embedded);setStatus(`앱 내장 자료 표시 중 · Excel 최신 파일을 확인합니다.`);}else{targets=[];projects=[];window.PRAYER_TARGETS=[];window.MISSION_PROJECTS=[];setStatus(`${UNIFIED_FILE} 최신 파일을 확인하는 중입니다.`);}try{apply(await fetchDefault())}catch(e){if(embedded&&Array.isArray(embedded.targets)){apply(embedded);setStatus(`모바일 안전모드: 앱 내장 자료를 표시합니다. · Excel 갱신 실패: ${e.message||e}`,false)}else{appSettings={};renderFilters();renderStats();renderList();renderProjects();setStatus(`자료 불러오기 실패: ${e.message||e}`,true)}}}
window.initPrayerMinistry=function(){['prayerSearch','prayerOwner','prayerGroup','prayerState'].forEach(id=>{const el=$('#'+id);if(el)el.addEventListener(id==='prayerSearch'?'input':'change',renderList)});$('#refreshPrayerExcel')?.addEventListener('click',load);$('#refreshEvangelismDuty')?.addEventListener('click',loadEvangelismDuties);$('#evangelismDutyMonth')?.addEventListener('change',renderEvangelismDuties);const goalInput=$('#soulGoal');if(goalInput){goalInput.readOnly=true;goalInput.title='통합 엑셀의 환경설정 시트에서 수정합니다.';}const saveGoal=$('#saveSoulGoal');if(saveGoal)saveGoal.style.display='none';$('#selectPrayerExcel')?.addEventListener('click',()=>$('#prayerExcelInput').click());$('#prayerExcelInput')?.addEventListener('change',async e=>{const f=e.target.files[0];if(!f)return;try{const sheets=await workbookSheets(await f.arrayBuffer());apply({name:f.name,targets:rowsToTargets(sheets['기도대상자관리']),projects:rowsToProjects(sheets['일만상상프로젝트']),settings:rowsToSettings(sheets['환경설정'])})}catch(err){setStatus(`파일 읽기 실패: ${err.message||err}`,true)}});$('#downloadPrayerTemplate')?.addEventListener('click',()=>{location.href='./'+encodeURIComponent(UNIFIED_FILE)});
let lastAutoReload=0;const retry=()=>{const now=Date.now();if(now-lastAutoReload<5000)return;lastAutoReload=now;load();loadEvangelismDuties();};
window.addEventListener('online',retry);
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')retry();});
load();loadEvangelismDuties();};
})();
