/* GESMS V10.6 - 군우명단 Excel 자동연동 (성별·출생연도·연령통계 지원) */
(function(){
  'use strict';
  ...
  async function parseXlsx(buffer){
    ...
    if(wb&&rels){
      ...
      sheets=[...wd.getElementsByTagNameNS('*','sheet')].map(s=>{
        ...
        let target=map.get(rid)||'worksheets/sheet1.xml';
    ...
    for(const sheet of sheets){
      ...
      if(!file) continue;
      const d=new DOMParser().parseFromString(await file.async('text'),'application/xml');
      const rows=[];
    ...
    throw new Error('군우명단 시트 또는 성명 열을 찾지 못했습니다.');
  }
  ...
  function parseYear(v){
    ...
    if(!m) return null;
    const y=Number(m[0]), now=new Date().getFullYear();
    return y>=1900&&y<=now ? y : null;
  }

  function normalizeGender(v){
    const s=norm(v);
    if(['남','남성','male','m'].includes(s)) return '남';
    if(['여','여성','female','f'].includes(s)) return '여';
    return text(v);
  }
  ...
  function buildData(rows){
    ...
    const ix={
      family:aliasIndex(h.headers,['가족구분','가족 구분','가족코드','가정코드','가족번호','가정번호','가족','가정','family']),
      head:aliasIndex(h.headers,['가족대표','가정대표','세대주','가족대표자']),
      name:aliasIndex(h.headers,['이름','성명','군우명','교인명']),
      gender:aliasIndex(h.headers,['성별','남녀','gender']),
      birthYear:aliasIndex(h.headers,['출생연도','출생년도','생년','태어난해','birthyear']),
    ...
    if(ix.family<0) throw new Error('군우명단에서 가족구분 열을 찾지 못했습니다.');
    if(ix.birthYear<0) throw new Error('군우명단에서 출생연도 열을 찾지 못했습니다.');
    ...
    for(let i=h.index+1;i<rows.length;i++){
      ...
      const birthday=monthDay(get(r,ix.solar)||get(r,ix.birth)||kind);
      const year=parseYear(get(r,ix.birthYear));
      members.push({
        family:(()=>{const code=get(r,ix.family);return code||`미분류-${name}`;})(),
        name,
        gender:normalizeGender(get(r,ix.gender)),
        birthYear:year,
        birthday,
    ...
    members.forEach(m=>{
      if(!famMap.has(m.family)) famMap.set(m.family,[]);
      famMap.get(m.family).push(m);
    });
    ...
    const positions={}, groups={}, birthMonths={}, genders={}, birthYears={};
    cleanMembers.forEach(m=>{
      ...
      const g=m.gender||'미기재';
      genders[g]=(genders[g]||0)+1;
      if(m.birthYear) birthYears[m.birthYear]=(birthYears[m.birthYear]||0)+1;
    });

    return {members:cleanMembers,families,stats:{positions,groups,birthMonths,genders,birthYears}};
  }
  ...
  window.loadLatestMemberExcel=async function(){
    try{
      ...
      window.GESMS_MEMBER_EXCEL={
        ok:true,name:wb.name,sheet:wb.sheet,
        count:fresh.members.length,familyCount:fresh.families.length,
        yearCount:fresh.members.filter(m=>m.birthYear).length
      };
      window.dispatchEvent(new CustomEvent('gesms-members-loaded',{detail:window.GESMS_MEMBER_EXCEL}));
      return true;
    }catch(e){
      window.GESMS_MEMBER_EXCEL={ok:false,error:e.message||String(e)};
      window.dispatchEvent(new CustomEvent('gesms-members-loaded',{detail:window.GESMS_MEMBER_EXCEL}));
      console.warn('[GESMS V10.6] 군우명단 Excel 연동 실패. data.js 예비명단 사용',e);
      return false;
    }
  };
})();
