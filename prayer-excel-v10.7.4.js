/* GESMS V10.7.4 - unified evangelism workbook + monthly project date preview */
(function(){
'use strict';
...
async function fetchDefault(){
  let last='통합 전도영혼구원 파일을 찾지 못했습니다.';
  for(const name of UNIFIED_FILES){
    try{const sheets=await workbookSheets(await fetchBuffer(name));return {name,targets:rowsToTargets(sheets['기도대상자관리']),projects:rowsToProjects(sheets['일만상상프로젝트']),settings:rowsToSettings(sheets['환경설정'])};}
    catch(e){last=e.message||String(e)}
  }
  for(const name of FALLBACK_PRAYER_FILES){try{const sheets=await workbookSheets(await fetchBuffer(name));const first=Object.values(sheets)[0]||[];return {name,targets:rowsToTargets(sheets['기도대상자관리']||first),projects:[],settings:{},warning:'통합 V3.1/V3.0 파일을 찾지 못해 기존 기도대상자 파일을 사용했습니다.'};}catch(e){last=e.message||String(e)}}
...
function renderProjects(){
  ...
  summary.innerHTML=`<div class="soul-kpis"><div><span>전체</span><b>${projects.length}</b></div><div><span>진행중</span><b>${counts['진행중']}</b></div><div><span>완료</span><b>${counts['완료']}</b></div><div><span>예산 집행</span><b>${budget?Math.round(spent/budget*100):0}%</b></div></div><div class="project-view-guide">📅 월을 선택하면 각 계획의 날짜와 사업명이 먼저 표시됩니다. 항목을 누르면 세부 일정과 내용을 볼 수 있습니다.</div>`;
...
function apply(data){targets=data.targets||[];projects=data.projects||[];appSettings=data.settings||{};appSettings['앱버전']=window.GESMS_APP_VERSION||appSettings['앱버전']||'자동연동';window.PRAYER_TARGETS=targets;window.MISSION_PROJECTS=projects;window.GESMS_SETTINGS=appSettings;renderFilters();renderStats();renderList();renderProjects();const goalSource=Number(appSettings['연간영혼구원목표'])>0?` · 연간목표 ${getGoal()}명(환경설정)`:'';setStatus(`${data.name} · 기도대상 ${targets.length}명 · 프로젝트 ${projects.length}건${goalSource}${data.warning?' · '+data.warning:''}`);}
